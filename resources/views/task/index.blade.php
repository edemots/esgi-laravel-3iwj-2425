                        ...
                        <x-dropdown>
                            ...
                            <x-slot name="content">
                                ...
                                <form action="{{ route('tasks.destroy', $task) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="block w-full px-4 py-2 text-start text-sm leading-5 text-red-700 dark:text-red-300 hover:bg-red-100 dark:hover:bg-red-800 focus:outline-none focus:bg-red-100 dark:focus:bg-red-800 transition duration-150 ease-in-out">{{ __('Delete task') }}</button>
                                </form>
                            </x-slot>
                        </x-dropdown>
                    </div>
            ...
            @empty
                <div class="flex min-w-0 flex-1 flex-col items-center justify-center gap-6 rounded-lg border-dashed p-6 text-center text-balance md:p-12 from-gray-600/50 to-bg-gray-900 h-full bg-gradient-to-b from-30%">
                    ...
                    <div class="flex max-w-sm flex-col items-center gap-2 text-center">
                        <div class="text-white text-lg font-medium tracking-tight">Aucune tâche à afficher</div>
                        <div class="text-gray-500 [&>a:hover]:text-primary text-sm/relaxed [&>a]:underline [&>a]:underline-offset-4">Vous n'avez pas encore créé de tâche. Créez-en une dès maintenant !</div>
                    </div>
                    <div class="flex w-full max-w-sm min-w-0 flex-col items-center gap-4 text-sm text-balance">
                        <a href="{{ route('tasks.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 dark:bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-white dark:text-gray-800 uppercase tracking-widest hover:bg-gray-700 dark:hover:bg-white focus:bg-gray-700 dark:focus:bg-white active:bg-gray-900 dark:active:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">{{ __('Create Task') }}</a>
                    </div>
                </div>
            @endforelse
        </section>
